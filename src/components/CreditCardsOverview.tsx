...
export function CreditCardsOverview({ cards, expenses, categories, onDeleteExpense, onEditExpense, addCard, updateCard, deleteCard , selectedMonth}: CreditCardsOverviewProps) {
  ...
  const handleSave = () => {
    ...
    if (editingCard) {
      updateCard(editingCard.id, { name, color, closing_day: closingDay, due_day: dueDay });
    } else {
      addCard({ name, color, closing_day: closingDay, due_day: dueDay });
    }
  ...
  return (
    <div className="w-full">
      ...
      {cards.length === 0 ? (
        <div className="flex flex-col items-center justify-center p-12 bg-slate-800/50 border border-dashed border-slate-700/50 rounded-3xl">
          ...
          <p className="text-sm text-slate-500 mt-1 mb-6 text-center max-w-xs">
            Adicione seus cartões de crédito para organizar suas faturas e descobrir o melhor dia para comprar.
          </p>
        </div>
      ) : (
          ...
          {cards.map(card => {
            ...
            return (
              <div 
                ...
                role="button" 
                className="relative overflow-hidden rounded-2xl p-6 border border-white/10 shadow-xl transition-all hover:scale-[1.02] cursor-pointer"
              ...
              >
                <div className="flex justify-between items-start mb-8">
                  ...
                  <div className="flex gap-2 opacity-50 hover:opacity-100 transition-opacity bg-black/20 rounded-lg p-1">
                    <button onClick={() => handleOpenModal(card)} className="p-1.5 text-white hover:text-emerald-300"><Edit2 className="w-4 h-4" /></button>
                    <button onClick={() => deleteCard(card.id)} className="p-1.5 text-white hover:text-rose-300"><Trash2 className="w-4 h-4" /></button>
                  </div>
                </div>
                
                <div className="flex justify-between items-end">
                  <div>
                    <p className="text-white/60 text-xs font-medium uppercase tracking-wider mb-1">Fechamento</p>
                    <p className="text-white font-bold text-lg drop-shadow-md">Dia {card.closing_day}</p>
                  </div>
                  <div>
                    <p className="text-white/60 text-xs font-medium uppercase tracking-wider mb-1 text-right">Vencimento</p>
                    <p className="text-white font-bold text-lg text-right drop-shadow-md">Dia {card.due_day}</p>
                  </div>
                </div>

                {isBestDay && (
                  <div className="absolute top-0 right-0 left-0 bg-emerald-500 text-white text-xs font-bold uppercase text-center py-1">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="bg-slate-800 border border-slate-700 rounded-3xl w-full max-w-md p-6 relative z-10 shadow-2xl animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-slate-400 mb-2">Apelido do Cartão</label>
                <input
                  type="text"
                  value={name}
                  onChange={e => setName(e.target.value)}
                  placeholder="Ex: Nubank, C6, Itau..."
                  className="w-full bg-slate-900/50 border border-slate-700/50 rounded-xl px-4 py-2.5 text-slate-100 focus:outline-none focus:ring-2 focus:ring-emerald-500/50"
                  maxLength={20}
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-slate-400 mb-2">Dia do Fechamento</label>
                  <input
                    type="number"
                    min="1" max="31"
                    value={closingDay}
                ...
                <div>
                  <label className="block text-sm font-medium text-slate-400 mb-2">Dia do Vencimento</label>
                  <input
                    type="number"
                    min="1" max="31"
                    value={dueDay}
